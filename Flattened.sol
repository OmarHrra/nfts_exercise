...
contract Dna {
  ...
  // Get attributes
  uint8 constant AND_SECTION_SIZE = 2;

  function getDnaSection(uint256 _dna, uint8 _rightDiscard) internal pure returns(uint8) {
    return uint8(
      (_dna % (1 * 10 ** (_rightDiscard + AND_SECTION_SIZE))) / (1 * 10 ** _rightDiscard)
    );
  }
...
// File contracts/ProjectPunks.sol
